...
# Load both datasets
current_data = pd.read_csv('football_data_co_uk/E0_24_25.csv')
previous_data = pd.read_csv('football_data_co_uk/E0_23_24.csv')

# Combine datasets
merged_data = pd.concat([previous_data, current_data], ignore_index=True)

# Check column names
print(merged_data.columns)

# Compute additional features if needed
merged_data['TotalShots'] = merged_data['HS'] + merged_data['AS']
merged_data['TotalShotsOnTarget'] = merged_data['HST'] + merged_data['AST']
merged_data['TotalCorners'] = merged_data['HC'] + merged_data['AC']
...
data = compute_rolling_avg(merged_data)
